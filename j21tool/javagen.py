...
def generate_java(tree, original_code, output_pathname):
    ...
    tree.types[0]._dirty = True
    with Formatter(open(output_pathname, "w", encoding="utf-8"), original_code) as out:
        print(f"out={out}")
        format_java(out, tree)
...
def format_any(out, elem):
    ...
    if isinstance(elem, Literal):
        ...
        out.add(elem.expression).add('::', PieceType.TIGHT_BINARY_OP)
        ...
        out.add(str(elem.operator), PieceType.BINARY_OP)
...
def format_annotations(out, annotations):
    """annotations"""
    if annotations:
